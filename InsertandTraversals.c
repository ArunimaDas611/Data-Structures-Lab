...
BSTNODE newNodeInBST(int item) {
	BSTNODE temp =
	    (BSTNODE)malloc(sizeof(struct node));
	temp->data = item;
...
BSTNODE insertNodeInBST(BSTNODE node, int
                        ele) {
	if(node == NULL)
	{
	...
	if(ele < node -> data)
	{
		node -> left = insertNodeInBST(node -
		                               > left, ele);
	}
	else if(ele > node -> data)
	{
		node -> right = insertNodeInBST(node
		                                -> right, ele);
	}
	else
	{
		printf("Element already exists in
		       BST");
	}
	return node;
}
BSTNODE searchNodeInBST(BSTNODE root, int
                        ele) {
	...
	if(ele > root -> data)
	{
		return searchNodeInBST(root ->
		                       right, ele);
	}
	else
	{
		return searchNodeInBST(root -> left,
		                       ele);
	}
}
